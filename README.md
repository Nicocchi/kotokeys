...
## Kanji and radical data

`src/data/kanji/kanji.json` holds one entry for every kanji that appears in the word lists (meanings, readings, stroke count, radical, and the Kanji alive media name plus per-stroke timings when a stroke-order video exists). `src/data/kanji/radicals.json` holds all 214 traditional radicals and their variants. Both files, and the media under `public/kanjialive/`, are generated by `outputs/jlpt_anki_extract/build_kanji_data.py`; the word generator reads `kanji.json` to fill each word's `kanjiBreakdown` meanings.

The word info panel can reveal the radical for each kanji and a stroke-order animation that can be stepped through stroke by stroke (both collapsed by default; use the toggles next to the Kanji heading). The **Radicals** button in the header opens a browsable list of every radical, and the **Words** button opens a searchable list of every word (filter by JLPT level; expand a word for its full details).
...
Kanji and radical information (meanings, readings, stroke counts, radicals and their positions), the kanji stroke-order videos, and the radical images, derivation animations, position icons, and radicals font come from [Kanji alive](https://kanjialive.com) via the [kanji-data-media](https://github.com/kanjialive/kanji-data-media) repository. This work by Kanji alive is licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/); the *Japanese Radicals* font is licensed under [Apache 2.0](http://www.apache.org/licenses/LICENSE-2.0.html). The copies bundled in `public/kanjialive/` and the data in `src/data/kanji/` are unmodified apart from being reformatted as JSON.

Kanji that are not covered by Kanji alive fall back to [KANJIDIC2](https://www.edrdg.org/wiki/index.php/KANJIDIC_Project), the property of the [Electronic Dictionary Research and Development Group](https://www.edrdg.org/), used in conformance with the Group's [licence](https://www.edrdg.org/edrdg/licence.html) (Creative Commons Attribution-ShareAlike 4.0). Because `src/data/kanji/kanji.json` includes KANJIDIC2-derived entries, that file is distributed under CC BY-SA 4.0.
